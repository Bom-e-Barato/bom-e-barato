...
export class CheckoutComponent implements OnInit {
  ...
  constructor(private _formBuilder: FormBuilder, public done_dialog: MatDialog, private _router: Router, private _service: SharedService, private _snackBar: MatSnackBar) {
  ...
  ngOnDestroy() {
    ...
    if (this.cart[0]?.type == 'promotion') this._service.openCartPage(null);
  }
  ...
  checkoutDone() {
    ...
    dialogRef.afterClosed().subscribe(_ => {
      var img = this.cart[0].event.img;
      this.cart[0].event.img = null;
      this._service.addProduct(this.cart[0].event).subscribe((data: any) => {
        console.log(data);
  ...
  productPrice(product: any) {
    if (product.type == 'key' || product.type == 'ticket' || product.type == 'promotion') {
  ...
  productType(product: any) {
    if (product.type == 'promotion') return 'Promover Anúncio';
